...
import datetime
# Asegúrate de importar tu StorageService
...
#     results = {'base_image': single_publicacion.rutaImg,
...
def filter_publicaciones():
    ...
    if fecha_start:
        query = query.filter(Publicacion.fecha >= datetime.datetime.strptime(
            fecha_start, '%Y-%m-%d').date())
    if fecha_end:
        query = query.filter(Publicacion.fecha <= datetime.datetime.strptime(
            fecha_end, '%Y-%m-%d').date())
...
#     query = db.session.query(Raza.raza, func.count(Publicacion.publicacionId).label(
#         'count')).outerjoin(Publicacion, Raza.id == Publicacion.razaId)
...
#         query = query.filter(Publicacion.usuarioId == usuario_id)
#     if ciudad_id:
#         query = query.join(Usuario, Publicacion.usuarioId ==
#                            Usuario.id).filter(Usuario.ciudadId == ciudad_id)
#     if publicacion_id:
#         query = query.filter(Publicacion.publicacionId == publicacion_id)
#     if tipo:
#         query = query.filter(Publicacion.tipo == tipo)
...
#     query = query.group_by(Raza.raza).order_by(
#         func.count(Publicacion.publicacionId).desc())

#     query = query.limit(15)

#     results = query.all()

#     raza_count_data = [{'raza': raza, 'count': count}
#                        for raza, count in results]
